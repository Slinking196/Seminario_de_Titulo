...
OLLAMA_MODEL_ID = "olmo-tesis:latest"
OLLAMA_NUM_CTX = 4096
OLLAMA_STOP_MARKERS = ["<|im_end|>", "<|endoftext|>", "```"]
...
def _build_user_prompt(data: str) -> str:
    return f"""Analiza la siguiente reseña:
            ---
            {data}
            ---
            Respuesta en JSON:"""
...
def _build_mlx_like_chatml_prompt(data: str) -> str:
    # Replica el template ChatML de MLX para OLMo-3 Think, incluyendo <think>.
    return "\n".join(
        [
            "<|im_start|>system",
            SYSTEM_PROMPT,
...
def _response_get(response: object, field: str, default: object = None) -> object:
...
def _extract_model_names(list_response: object) -> set[str]:
    ...
    models = _response_get(list_response, "models", [])
...
def ensure_ollama_model(client: ollama.Client, model_id: str) -> None:
    try:
        list_response = client.list()
        available_models = _extract_model_names(list_response)
    except Exception as e:
        raise RuntimeError(
            "No fue posible conectar con el servidor de Ollama. "
            f"Verifica que esté activo en {OLLAMA_HOST}. Error: {e}"
        ) from e

    if model_id in available_models:
        log_progress(f"Modelo Ollama disponible: {model_id}")
        return

    raise RuntimeError(
        f"Modelo '{model_id}' no está disponible en Ollama. "
        f"Ejecuta: ollama pull {model_id}"
    )


def analyze_data(data: str, client: ollama.Client, model_id: str) -> dict | None:
    prompt = _build_mlx_like_chatml_prompt(data)
    ...
    try:
        response = client.generate(
            ...
            options={
                ...
                "temperature": 1,
                "top_p": 0.85,
                "stop": OLLAMA_STOP_MARKERS,
            },
        )
    except Exception as e:
        log_progress(f"Error en inferencia Ollama: {e}")
        return None

    response_text = str(_response_get(response, "response", "")).strip()

    extracted_json = extract_json_from_olmo(response_text)
    if extracted_json is None:
        log_progress("-" * 100)
        log_progress(f"Comentario enviado a OLMo3: {data}")
        log_progress(f"Respuesta OLMo3: {response_text}")
        log_progress("-" * 100)
...
def label_reviews() -> None:
    ...
    df = df[3000:8000]  # Limitar a 10000 filas para pruebas iniciales
    ...
    log_progress("Conectando con servidor Ollama", total_start)
    log_progress(
        f"Configuracion Ollama | host={OLLAMA_HOST} | model={OLLAMA_MODEL_ID} | "
        f"max_new_tokens={MAX_NEW_TOKENS}"
    )
    client = ollama.Client(host=OLLAMA_HOST)
    ensure_ollama_model(client, OLLAMA_MODEL_ID)
            ...
            for processed_count, (idx, review_text, lang_value) in enumerate(
                ...
                if not cleaned_text:
                    ...
                    if cleaned_text in text_cache:
                        ...
                        parsed = analyze_data(cleaned_text, client, OLLAMA_MODEL_ID)
